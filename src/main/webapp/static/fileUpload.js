...
var uploadTools = {
     ...
     */
    "getShowFileType":function(isImg,fileType,fileName,isImgUrl,fileCodeId,opt){
        ...
        //pictype=[{"code":"1","name":"人物"},{"code":"2","name":"动物"}]
        if(opt.pictype.length>0){
          var _select="<select>";
            for(var i=0;i<opt.pictype.length;i++){
                _select=_select+'<option value="'+opt.pictype[i].code+'">'+opt.pictype[i].name+'</option>'
            }                
        ...
        for(var i=0;i<fileList.length;i++){
            ...
            if(fileTypeArray=="*"||uploadTools.isInArray(fileTypeStr,fileTypeArray)){
                ...
                if(imgtest.test(fileList[i].type)){
                    ...
                    var fileModel = uploadTools.getShowFileType(true,fileTypeUpcaseStr,fileList[i].name,imgUrlStr,fileListArray.length,opt);
                    $(boxJsObj).append(fileModel);
                }else{
                    var fileModel = uploadTools.getShowFileType(true,fileTypeUpcaseStr,fileList[i].name,null,fileListArray.length,opt);
                    $(boxJsObj).append(fileModel);
                }
    ...
    "uploadFile":function(opt){
        ...
        for(var i=0;i<fileList.length;i++){
            if(fileList[i]!=null){
                ...
                //如果上传组件，有select 选择框，那么增加选择值
                if($("div[filecodeid]").find("select").length>0){
                	pictypes=pictypes+$("div[filecodeid='"+i+"']").find("select").find("option:selected").val()+",";
                }
            }
        }
        if(pictypes!=""){
        	pictypes=pictypes.substring(0,pictypes.length-1);
        }
        formData.append("pictypes",pictypes); 
